...
(function() {
  ...
  // Acepta File o Blob
  function compressImage(fileOrBlob, maxSize = 1024, quality = 0.8) {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        img.onload = () => {
          ...
          const originalSize = fileOrBlob.size ? Math.round(fileOrBlob.size / 1024) + 'KB' : 'unknown';
  ...
  const styles = `
    * {
    ...
    .tryon-panel {
      ...
      position: fixed;
      bottom: 85px; /* 24px (FAB bottom) + 56px (FAB height) + 5px gap */
      right: 24px;
      top: auto;
      left: auto;
      width: 380px;
      max-width: calc(100vw - 48px);
      max-height: calc(100vh - 85px - 20px); /* Hasta arriba con 20px de margen */
      background: white;
      border-radius: 16px;
    ...
    /* Panel expanded state when showing result */
    ...
    <div class="tryon-panel" id="tryon-panel" style="display: none;">
      ...
      <div class="tryon-body">
        ...
        <div class="tryon-inputs-used" id="inputs-used">
          <span class="tryon-inputs-used-label">Inputs Used</span>
          <div class="tryon-inputs-thumbnails" id="inputs-thumbnails">
            <!-- Populated dynamically: 1 base + 3 garment slots -->
          </div>
        </div>
      </div>
  ...
  function renderInputsUsed() {
    ...
    if (state.inputsUsed.baseImage) {
      html += `<div class="tryon-input-thumb"><img src="${state.inputsUsed.baseImage}" alt="Base"></div>`;
    }
    
    // Garment thumbnails (always show 3 slots)
    for (let i = 0; i < 3; i++) {
      const garment = state.inputsUsed.garments[i];
      if (garment) {
        html += `<div class="tryon-input-thumb"><img src="${garment}" alt="Garment ${i + 1}"></div>`;
      } else {
        html += `<div class="tryon-input-thumb empty"></div>`;
      }
    }
  ...
  async function generateImage() {
    // If already completed, "Try another look" reuses existing inputs
    if (state.generationStatus === 'completed') {
      // Reset to idle but keep inputs - user can modify them if needed
