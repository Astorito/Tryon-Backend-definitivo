...
(function() {
  ...
  const styles = `
    * {
    ...
    .tryon-input-thumb:hover {
      border-color: #667eea;
    }

    .tryon-input-thumb.active {
      border-color: #667eea;
    ...
    .tryon-input-thumb.add-more:hover {
      border-color: #667eea;
      color: #667eea;
    }

    /* Try another look button */
    .tryon-try-another-btn {
      display: none;
      width: calc(100% - 32px);
    ...
    .tryon-try-another-btn:hover {
      background: linear-gradient(90deg, #d97706, #ea580c);
    }

    .tryon-try-another-btn.show {
      display: block;
    }
    ...
    <div class="tryon-panel" id="tryon-panel" style="display: none;">
      ...
      <div class="tryon-inputs-used" id="inputs-used">
        <div class="tryon-inputs-thumbnails" id="inputs-thumbnails">
          <!-- Populated dynamically: base + garments + add button -->
        </div>
      </div>
      
      <button class="tryon-try-another-btn" id="try-another-btn">Try another look</button>
  ...
  const tryAnotherBtn = shadow.getElementById('try-another-btn');

  // Try another look button handler
  ...
  function renderInputsUsed() {
    if (!state.inputsUsed) {
      ...
      tryAnotherBtn.classList.remove('show');
      return;
    }
    ...
    if (state.inputsUsed.baseImage) {
      html += `<div class="tryon-input-thumb" title="Your photo"><img src="${state.inputsUsed.baseImage}" alt="Base"></div>`;
    }
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (garment) {
        html += `<div class="tryon-input-thumb active" title="Garment ${i + 1}"><img src="${garment}" alt="Garment ${i + 1}"></div>`;
      }
    }
    
    // Add more button
    html += `<div class="tryon-input-thumb add-more" id="add-more-garment" title="Add another garment">+</div>`;
    
    inputsThumbnails.innerHTML = html;
    inputsUsed.classList.add('show');
    tryAnotherBtn.classList.add('show');
    
    // Setup add more click handler
    const addMoreBtn = shadow.getElementById('add-more-garment');
    if (addMoreBtn) {
      addMoreBtn.onclick = () => {
        resetResultView();
      };
