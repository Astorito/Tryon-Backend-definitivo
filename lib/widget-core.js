...
(function() {
  ...
  // Cerrar al hacer click fuera del panel (sin overlay bloqueante)
  // Usamos mousedown en document para detectar clicks fuera
  document.addEventListener('mousedown', (e) => {
    if (!state.isOpen || state.isDragging) return;
    
    // Verificar si el click fue dentro del panel o FAB
    const path = e.composedPath();
    const clickedInsidePanel = path.includes(panel);
    const clickedInsideFab = path.includes(fab);
    
    // Si el click fue fuera del panel y del FAB, cerrar
    if (!clickedInsidePanel && !clickedInsideFab) {
      closeWidget();
    }
  });
