...
(function() {
  ...
  function renderInputsUsed() {
    ...
    // Add "+" button to add more garments
    html += `<div class="tryon-input-thumb empty" id="add-garment-thumb" title="Add another garment"></div>`;
    
    inputsThumbnails.innerHTML = html;
    inputsUsed.classList.add('show');
    
    // Setup click handler for add button
    const addBtn = shadow.getElementById('add-garment-thumb');
    if (addBtn) {
      addBtn.onclick = () => {
        resetResultView();
      };
